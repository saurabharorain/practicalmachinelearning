...
removeredunt <- trainset %>% select(-X,-raw_timestamp_part_1,-raw_timestamp_part_2,-cvtd_timestamp,-new_window,-num_window,-user_name,-classe)
...
set.seed(1234)
#baserpart.rds
model <- train(classe~.,data=trainsetwithoutna,method="rpart");

#baserpart-pca.rds
#model <- train(classe~.,data=trainsetwithoutna,method="rpart",preProcess=c("pca"));
...
saveRDS(model,file="baserpart.rds")
